...
const App: React.FC = () => {
  ...
  const handleSaveCard = (data: { title: string; content: string; category: Category }, id?: string) => {
    setCards(prevCards => {
  ...
  const getCatIcon = (cat: Category) => {
    switch (cat) {
      case SystemCategories.MEDICAL: return <MedicalIcon />;
      case SystemCategories.DAILY: return <DailyIcon />;
      case SystemCategories.SERVICES: return <ServiceIcon />;
      case SystemCategories.EMERGENCY: return <CrossIcon />;
      default: return <div className="text-zinc-600 font-bold">{cat[0]?.toUpperCase() || '?'}</div>;
    }
  };

  const otherCats = categories.filter(c => c !== SystemCategories.EMERGENCY);
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-40 landscape:pb-24 select-none">
      ...
      <nav className="sticky top-[108px] landscape:top-[60px] z-30 bg-black/95 backdrop-blur-xl py-4 landscape:py-2 overflow-x-auto whitespace-nowrap px-6 no-scrollbar flex items-center gap-2">
        <button
      ...
      <main className="px-6 mt-6 landscape:mt-2" role="list">
